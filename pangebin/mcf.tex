...
\begin{definition}{\MCF{} MILP variables}{milp:mcf_variables}
  \begin{itemize}
    ...
    \item \(\beta_a \in \Reals_{\leq 0} \, \forall a \in A_\Links{}\) is strictly negative if the link-arc \(a\) participates in one of the solution subgraph's exploration tree.
      It acts as a negative integer, where the absolute value correspond to the depth of the subtree defined by the successor.
  \end{itemize}
  Variables \(\alpha{}\) and the \(\beta_{A_\Links{}}\) serves to model the solution subgraph connectivity.
\end{definition}
...
\begin{definition}{\MCF{} MILP constraints}{milp:mcf_constraints}
  ...
  \begin{equation}
    \begin{split}
      f_{uv} \leq \min\Set*{\cov{i}, \cov{j}} y_{uv} \quad \forall (u, v) \in A_\Links{} \\
      i, j = vfrag(u), vfrag(v)
    \end{split}
  \end{equation}
