...
\begin{todobox}
  CONTINUE HERE % #TODO CONTINUE HERE (change constraint list, they differ from decomp and binlab quite a lot)
\end{todobox}

\begin{table}[h!]
  \centering
  \tablecaption{\MGCLB{} MILP constraints}{What is new here comparing to the two previous approaches is we need to define variables \(\inflowgc{i}{b}\) for all fragments \(i \in \Fragments{}\) and GC content interval \(b \in K\).}\label{tab:once:mgclb:cst}
  \begin{tabular}{@{}ll@{}}
    \toprule
