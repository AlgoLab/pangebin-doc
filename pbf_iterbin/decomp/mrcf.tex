...
The solution set \(S_\MPS{}\) for \MPS{} contains solutions with the same value of \(PlasmidnessScore\) and different values for \(CoverageFlow\) and \(GCScore\) because of \zcref[S]{pbf_iterbin:decomp:mgc:cst:fix_mcf_obj,pbf_iterbin:decomp:mps:cst:fix_mgc_obj}.
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    \midrule
    \multicolumn{2}{@{}l@{}}{Constraints in \zcref[S]{tab:decomp:mps:cst}} \\
    \addlinespace
    \MRCF{} dedicated & \zcref[S]{pbf_iterbin:decomp:mrcf:cst:fix_mps_obj} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{Constraint}
  \begin{equation}
    PlasmidnessScore = P
    \label{pbf_iterbin:decomp:mrcf:cst:fix_mps_obj} % chktex 25
  \end{equation}
\end{Constraint}
...
\begin{definition}{\MRCF{} MILP objective function}{pbf_iterbin:decomp:mrcf:obj}
  ...
  \begin{Objective}
    \begin{equation}
      \max ~ |\Contigs{}| CoverageFlow + GCScore
      \label{pbf_iterbin:decomp:mrcf:max_coverage_and_gc_score} % chktex 25 % chktex 35
    \end{equation}
  \end{Objective}
\end{definition}
